...
class LocaliteReport(models.Model):
    """ Mission Analysis (LOCALITE) """

    _name = "cim.localite.mission.report"
    _auto = False
    _description = "Analyse de localité"
    _rec_name = 'id'

    date_arrivee = fields.Datetime("Date d'arrivée", readonly=True)
    loc_id = fields.Many2one("cim.localite", string="Localité", readonly=True)
    duree = fields.Integer("Durée (en jour)", readonly=True)
    ...
    duree_mission = fields.Integer(string="Durée de mission (en jour)", readonly=True)
    ...
    def _from(self):
        return """
            FROM cim_mission_itineraire AS i
        """

    def _join(self):
        return """
            JOIN cim_mission AS c ON i.mission_id = c.id
        """
